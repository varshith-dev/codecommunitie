...
export default function Feed({ session }) {
  ...
  const getSortedPosts = () => {
    if (activeTab === 'trending') {
      return [...posts].sort((a, b) => {
        // Hacker News Gravity Formula: Score = (P - 1) / (T + 2)^G
        // P = points (likes + comments*2)
        // T = time in hours since submission
        // G = gravity (1.8 default)
        ...
        const gravity = 1.8
        const rankA = (scoreA + 1) / Math.pow(hoursA + 2, gravity)
        const rankB = (scoreB + 1) / Math.pow(hoursB + 2, gravity)
  ...
  return (
    <div className="max-w-full mx-auto pb-20">
      ...
      {ads.find(a => a.placement === 'banner') && activeTab === 'latest' && (
        <div className="mb-6">
          <AdCard ad={ads.find(a => a.placement === 'banner')} />
        </div>
      )}
      ...
      {displayedPosts.map((post, index) => (
        <div key={post.id}>
          ...
          {index > 0 && index % 3 === 0 && feedAds.length > 0 && (
            <AdCard ad={feedAds[(index / 3 - 1) % feedAds.length]} />
          )}
